...
const PricePartitionPage = ({ navigate, quotationConfig, setQuotationConfig }) => {
  ...
  const [partitions, setPartitions] = useState([
    { id: Date.now(), name: '', applyNo: '', contractCode: '', fromCity: '', toCity: '', statGroup: '', feeItem: '', orderType: '', deliveryType: '', direction: '' }
  ]);
  ...
  const isStat = quotationConfig?.baseInfo?.quotationMethod === '产品报价' && (quotationConfig?.baseInfo?.billingStrategy === '统计考核' || quotationConfig?.baseInfo?.billingStrategy === '统计+合单');

  const handleAddPartition = () => {
    setPartitions([...partitions, { id: Date.now(), name: '', applyNo: '', contractCode: '', fromCity: '', toCity: '', statGroup: '', feeItem: '', orderType: '', deliveryType: '', direction: '' }]);
  };
