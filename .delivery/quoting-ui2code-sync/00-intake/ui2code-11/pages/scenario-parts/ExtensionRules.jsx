...
const ExtensionRules = ({ data, onChange, formData }) => {
  ...
  const DropdownMultiSelect = ({ label, tooltip, options, value = [], onChange, single }) => {
    ...
    const validValue = (value || []).filter(v => options.includes(v));
    ...
    const toggleOption = (opt) => {
      if (single) {
        ...
        if (validValue.includes(opt)) {
          onChange(validValue.filter(v => v !== opt));
        } else {
          onChange([...validValue, opt]);
        }
      }
    };
      ...
      <div className="flex items-start mb-4 text-sm" ref={containerRef} data-ai-changelog-id="extension-dropdown-multi-select" data-ai-changelog-title="扩展点字段下拉多选" data-ai-changelog-desc="配置字段从平铺标签调整为下拉多选组件">
        ...
        <div className="flex-1 relative">
          <div 
            ...
            data-ai-alt="下拉多选触发区"
          >
            {validValue.length === 0 && <span className="text-gray-400 pl-2">请选择</span>}
            {validValue.map(opt => (
  ...
  const strategy = formData?.baseInfo?.strategy || '普通';
  const quotationMode = formData?.baseInfo?.quotationMode || '业务对象报价';
  const isStatRequired = ['统计考核', '统计+合单'].includes(strategy);
  const isMergeRequired = ['合单计费', '统计+合单'].includes(strategy);
  const hideStatRule = quotationMode === '产品报价' && isStatRequired;

  const forceEnableStat = (isOnlyStat || isStatRequired) && !hideStatRule;
  const forceDisableStat = (isOnlyBill && !isStatRequired) || hideStatRule;

  useEffect(() => {
    let newEnableStat = data.enableStat;
    let newEnableMerge = data.enableMerge;
    let changed = false;

    if (forceEnableStat && !data.enableStat) {
      newEnableStat = true;
      changed = true;
    } else if (forceDisableStat && data.enableStat) {
      newEnableStat = false;
      changed = true;
    }

    if (isMergeRequired && !data.enableMerge) {
      newEnableMerge = true;
      changed = true;
    }

    if (changed) {
      onChange({ ...data, enableStat: newEnableStat, enableMerge: newEnableMerge });
    }
  }, [forceEnableStat, forceDisableStat, isMergeRequired, data.enableStat, data.enableMerge, data, onChange]);
        ...
        <div className="border border-gray-200 rounded-lg p-5 bg-gray-50 transition-all">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
                 ...
                 <button 
                    onClick={() => {
                      if (isMergeRequired) return;
                      handleChange('enableMerge', !data.enableMerge);
                    }}
                    disabled={isMergeRequired}
                    className={`w-12 h-6 rounded-full p-1 transition-colors relative ${data.enableMerge ? 'bg-blue-600' : 'bg-gray-300'} ${isMergeRequired ? 'opacity-50 cursor-not-allowed' : ''}`}
                 >
        ...
        <div className="border border-gray-200 rounded-lg p-5 bg-gray-50 transition-all">
           <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button 
                        onClick={() => {
                            if (forceDisableStat || forceEnableStat) return;
                            handleChange('enableStat', !data.enableStat);
                        }}
                        disabled={forceDisableStat || forceEnableStat}
                        className={`w-12 h-6 rounded-full p-1 transition-colors relative ${data.enableStat ? 'bg-indigo-600' : 'bg-gray-300'} ${(forceDisableStat || forceEnableStat) ? 'opacity-50 cursor-not-allowed' : ''}`}
                        data-ai-alt="统计规则开关"
                    >
          ...
          {data.enableStat && (
              <div className="bg-white rounded p-4 shadow-sm space-y-2 animate-fade-in">
                ...
                <DropdownMultiSelect
                label="统计对象"
                tooltip="统计的对象类型，如按单据或按金额统计"
                options={['月度单量', '月度金额']}
                value={data.statTarget || []}
                onChange={(val) => handleChange('statTarget', val)}
                />
                ...
                label="开闭区间类型"
                tooltip="定义数值区间的包含关系"
                options={['左闭右开', '左开右闭', '双闭', '双开']}
                value={data.intervalType ? [data.intervalType] : []}
                onChange={(val) => handleChange('intervalType', val[0])}
                single={true}
                />
             </div>
          )}
